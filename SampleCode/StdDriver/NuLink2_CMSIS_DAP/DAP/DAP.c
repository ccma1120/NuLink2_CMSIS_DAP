...
static uint8_t DAP_Info(uint8_t id, uint8_t *info) {
  ...
  switch (id) {
    ...
    case DAP_ID_PACKET_SIZE:
      info[0] = (uint8_t)(DAP_GetPacketSize() >> 0);
      info[1] = (uint8_t)(DAP_GetPacketSize() >> 8);
      length = 2U;
      break;
...
static uint32_t g_DAP_PacketSize = DAP_PACKET_SIZE;

void DAP_SetPacketSize(uint32_t size)
{
    g_DAP_PacketSize = size;
}

uint32_t DAP_GetPacketSize(void)
{
    return g_DAP_PacketSize;
}
