 ...
 * @file     hid_transfer.c
 * @version  V1.00
 * @brief    Nu-Link2 CMSIS-DAP Sample file
 *
...
static uint8_t g_u8OutBuf[1024];
static uint8_t g_u8InBuf[1024];

/* CMSIS-DAP */
#include "DAP\DAP.h"
#include "DAP_config.h"

static uint8_t g_USB_Request[DAP_PACKET_COUNT][DAP_PACKET_SIZE];
static uint32_t g_free_count;
static uint32_t g_send_count;
static uint32_t g_recv_idx;
static uint32_t g_send_idx;
...
void USBD20_IRQHandler(void)
{
    ...
    if (IrqStL & HSUSBD_GINTSTS_EPAIF_Msk)
    {
        ...
        if (HSUSBD->EP[EPA].EPINTSTS & HSUSBD_EPINTEN_BUFEMPTYIEN_Msk)
            EPA_Handler();
    ...
    if (IrqStL & HSUSBD_GINTSTS_EPBIF_Msk)
    {
        ...
        if (HSUSBD->EP[EPB].EPINTSTS & HSUSBD_EPINTEN_BUFFULLIEN_Msk)
            EPB_Handler();
...
void EPA_Handler(void)  /* Interrupt IN handler */
{
    ...
    if (g_send_count > 0)
    {
        u32Len = DAP_GetPacketSize();

        HID_SetInReport(g_u8InBuf, u32Len);

        for (i = 0; i < u32Len; i++)
            HSUSBD->EP[EPA].EPDAT_BYTE = g_u8InBuf[i];
...
void EPB_Handler(void)  /* Interrupt OUT handler */
{
    ...
    for (i = 0; i < u32Len; i++)
        g_u8OutBuf[i] = HSUSBD->EP[EPB].EPDAT_BYTE;

    HID_GetOutReport(g_u8OutBuf, u32Len);
...
void HID_InitForHighSpeed(void)
{
    ...
    HSUSBD_SetEpBufAddr(EPA, EPA_BUF_BASE, EPA_MAX_PKT_SIZE);
    ...
    HSUSBD_SetEpBufAddr(EPB, EPB_BUF_BASE, EPB_MAX_PKT_SIZE);
    ...
    DAP_SetPacketSize(DAP_PACKET_SIZE_HS);
}
...
void HID_InitForFullSpeed(void)
{
    /*****************************************************/
    /* EPA ==> Interrupt IN endpoint, address 1 */
    HSUSBD_SetEpBufAddr(EPA, EPA_BUF_BASE, EPA_OTHER_MAX_PKT_SIZE);
    ...
    /* EPB ==> Interrupt OUT endpoint, address 2 */
    HSUSBD_SetEpBufAddr(EPB, EPB_BUF_BASE, EPB_OTHER_MAX_PKT_SIZE);
    HSUSBD_SET_MAX_PAYLOAD(EPB, EPB_OTHER_MAX_PKT_SIZE);
    HSUSBD_ConfigEp(EPB, INT_OUT_EP_NUM, HSUSBD_EP_CFG_TYPE_INT, HSUSBD_EP_CFG_DIR_OUT);
    HSUSBD_ENABLE_EP_INT(EPB, HSUSBD_EPINTEN_RXPKIEN_Msk | HSUSBD_EPINTEN_BUFFULLIEN_Msk);
    g_u32EpBMaxPacketSize = EPB_OTHER_MAX_PKT_SIZE;

    DAP_SetPacketSize(DAP_PACKET_SIZE_FS);
}
...
void HID_GetOutReport(uint8_t *pu8EpBuf, uint32_t u32Size)
{
    if (g_free_count > 0)
    {
        ...
        memcpy(g_USB_Request[g_recv_idx], pu8EpBuf, u32Size);

        DAP_ExecuteCommand(pu8EpBuf, g_USB_Request[g_recv_idx]);

        g_recv_idx = (g_recv_idx + 1) % DAP_PACKET_COUNT;
        g_send_count++;
    }
}

void HID_SetInReport(uint8_t *pu8EpBuf, uint32_t u32Size)
{
    if (g_send_count > 0)
    {
        g_send_count--;

        memcpy(pu8EpBuf, g_USB_Request[g_send_idx], u32Size);

        g_send_idx = (g_send_idx + 1) % DAP_PACKET_COUNT;
        g_free_count++;
    }
}
